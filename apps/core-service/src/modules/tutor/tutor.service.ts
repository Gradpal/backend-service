...
export class TutorService {
  ...
  async updateProfile(
    ...
    delete (updateDataWithoutFiles as any).academic_transcript;
    delete (updateDataWithoutFiles as any).degree_certificate;
    ...
    if (updateDataWithoutFiles.religious_affiliation) {
      updateDataWithoutFiles.religious_affiliation = JSON.parse(
        updateDataWithoutFiles.religious_affiliation as any,
      );
      (updateDataWithoutFiles as any).religious_affiliation = {
        value: updateDataWithoutFiles.religious_affiliation.value,
        visible: updateDataWithoutFiles.religious_affiliation.visible,
      };
    }
    ...
    if (files && files.length > 0) {
      ...
      if (groupedFiles['introductory_video']?.[0]) {
        const videoUrl = await this.minioService.uploadFile(
          groupedFiles['introductory_video'][0],
        );
        tutor.introductory_video = videoUrl;
      }
      ...
      if (updateDataWithoutFiles.institutions) {
        const institutions = updateDataWithoutFiles.institutions.map(
          (institution) => ({
            ...
            degree_type: institution.degree_type,
            year_started: institution.start_year,
            year_ended: institution.end_year,
        ...
        const transcriptFiles = files.filter(
          (f) =>
            ...
            f.fieldname.includes('][academic_transcript]'),
        );

        for (const file of transcriptFiles) {
          const match = file.fieldname.match(
            /institutions\[(\d+)\]\[academic_transcript\]/,
          );
          if (match && institutions[parseInt(match[1])]) {
            const transcriptUrl = await this.minioService.uploadFile(file);
            institutions[parseInt(match[1])].academic_transcript =
              transcriptUrl;
          }
        }
        ...
        const certificateFiles = files.filter(
          (f) =>
            f.fieldname.startsWith('institutions[') &&
            f.fieldname.includes('][degree_certificate]'),
        );

        for (const file of certificateFiles) {
          const match = file.fieldname.match(
            /institutions\[(\d+)\]\[degree_certificate\]/,
          );
          if (match && institutions[parseInt(match[1])]) {
            const certificateUrl = await this.minioService.uploadFile(file);
            institutions[parseInt(match[1])].degree_certificate =
              certificateUrl;
          }
        }

        tutor.institutions = institutions;
      }
    }

    if (updateDataWithoutFiles.country_of_residence) {
      tutor.country_of_residence = updateDataWithoutFiles.country_of_residence;
    }
    if (updateDataWithoutFiles.time_zone) {
      tutor.timezone = updateDataWithoutFiles.time_zone;
    }
    if (updateDataWithoutFiles.time_zone_display_format) {
      tutor.time_zone_display_format =
